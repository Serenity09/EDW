library Collectible requires Alloc, PermanentAlloc, SimpleList, Teams, IStartable, Levels, Deferred, All, TimerUtils, locust, UnitDefaultRadius
	globals
	...
	struct Collectible extends array
		...
		public static method create takes integer uncollectedUnitID, real x, real y, real facing returns thistype
			...
			return new
		endmethod
		public static method createFromPoint takes integer uncollectedUnitID, rect loc, real facing returns thistype
			return thistype.create(uncollectedUnitID, GetRectCenterX(loc), GetRectCenterY(loc), facing)
		endmethod
	endstruct
						...
						exitwhen curCollectibleNode == 0
							...
							if SquareRoot(deltaX*deltaX + deltaY*deltaY) <= Collectible(curCollectibleNode.value).UncollectedUnitRadius + User(curPlayerNode.value).ActiveUnitRadius then
								if not curCollectibleDeferred.Resolved then
									call CollectibleAcquireEffect(curCollectibleNode.value, curPlayerNode.value)
									call CollectibleTeam(curTeamNode.value).Team.SetUnitLocalOpacityForTeam(Collectible(curCollectibleNode.value).UncollectedUnit, INACTIVE_UNIT_OPACITY)
									
									call curCollectibleDeferred.Resolve(curPlayerNode.value)
								endif
								
								if Collectible(curCollectibleNode.value).ReturnToCheckpoint then
									call TeleportEffect(Collectible(curCollectibleNode.value).UncollectedUnit, curPlayerNode.value)
