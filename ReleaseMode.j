library ConfigurationMode requires GameModesGlobals
	globals
		...
		constant boolean FORCE_SETTING_MENU = true //when true, the initial visual vote menu will always be visible, even when CONFIGURATION_PROFILE != RELEASE
	...
	function GetFirstLevelID takes nothing returns Levels_Level
        if DEBUG_MODE and CONFIGURATION_PROFILE != RELEASE then
            //3 == first ice level
